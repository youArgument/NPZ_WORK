...
class SectorViewer(tk.Tk):
    ...
    def _plot_sector(self, sector_data: np.ndarray) -> None:
        ...
        # Визуализация сетки тайлов для выбранного сектора.
        # Рисуем только в координатах текущего сектора (x=distance строки, y=датчики).
        tile_px = max(1, int(self.tile_size_var.get()))
        overlap = float(self.overlap_ratio_var.get())
        step = max(1, int(tile_px * (1.0 - overlap)))
        # sector_data: [rows(distance), sensors]
        rows = sector_data.shape[0]
        cols = sector_data.shape[1]

        # Сетка по оси X (distance/строки)
        for x0 in range(0, rows, step):
            x1 = min(x0 + tile_px, rows)
            # Прямоугольник тайла по области сектора
